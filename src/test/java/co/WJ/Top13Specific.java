...
public class Top13Specific extends WebDriver {
    ...
    public void Top13_XC2192K() throws Exception { //id 1,3
        ...
        driver.get("https://zales.wonderjewel.co/configurator/XC2192K"); //Открыть урлу
        ...
        dropdown.selectByVisibleText("1/3 CT."); //Выбрать из выпадающего списка

        Thread.sleep(8000);

        dropdown = new Select(driver.findElement(new By.ByName("size"))); //drop-down list
        dropdown.selectByVisibleText("1/2 CT.");

        Thread.sleep(8000);

        if (driver.getPageSource().contains("3,699.99")) { //Проверка цены
            System.out.println("- Cost Done (XC2192K 3,699.99)");
        } else {
            int number = new Random().nextInt(1000) + 5; //Рандомайзер

            File scrFile = ((TakesScreenshot) driver).getScreenshotAs(OutputType.FILE);
            FileUtils.copyFile(scrFile, new File("C:\\screenshot\\screenshot" + number + ".png")); //Если неверная цена - Скрин

            System.out.println("- Screenshot (XC2192K 3,699.99)");
        }

        ((JavascriptExecutor) driver).executeScript("arguments[0].style.display = 'block';", driver.findElement(By.id("OV"))); //выбираем Овал (открыли элемент)
        driver.findElement(By.id("OV")).click();

        Thread.sleep(8000);

        if (driver.getPageSource().contains("4,499.99")) { //Проверка цены
    ...
    public void Top13_XC1222K() throws Exception { //id 10
        ...
        new Select(driver.findElement(new By.ByName("size"))).selectByVisibleText("1/3 CT.");
        Select dropdown = new Select(driver.findElement(new By.ByName("size")));
        dropdown.selectByVisibleText("1/3 CT.");
    ...
    public void Top13_XC1061() throws Exception { //id 19,20,21,22
        ...
        new Select(driver.findElement(new By.ByName("size"))).selectByVisibleText("1 CT.");
        Select dropdown = new Select(driver.findElement(new By.ByName("size")));
        dropdown.selectByVisibleText("1 CT.");
